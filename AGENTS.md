...
As of 2026-05-22, the benchmark is not complete.

- Hard gate 8 has a passing implementation path: `run-release-validation` produced raw simulator evidence for direct-release fall, released SONIC `CONTROL` standing for more than 5 seconds, and fall after stopping control.
- Hard gate 11 is partially implemented: simulator runs now log replay data with root `qpos`, support state, timing, joint order metadata, and measured joints; report rendering can produce a MuJoCo side-by-side MP4 from those logs for one real motion pair. Full 10-motion video/report completion is not yet verified.
- Hard gate 12 is newly added and not yet complete. The current task is to make HoloMotion and SONIC use the same Unitree G1 29DOF simulator/robot interface so the benchmark can be swapped to one real 29DOF robot without policy-specific simulator plants.
- Current known mismatch to eliminate: SONIC previously launched against a `scene_43dof.xml`/`g1_29dof_with_hand.xml` simulator scene, while HoloMotion used a 29DOF scene. That violates the single-robot hard gate even if policy action output remains 29 body DOF.
- Existing partial logs under `logs/` are development evidence only unless regenerated by `run.sh` with the current code and validated by `report.sh`.
