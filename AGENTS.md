...
- Fresh evidence in `logs_simplified_full2/` and `artifacts_simplified_full2_rerun/` passed report checks: `all_rmse_passed=true`, `all_release_gates_passed=true`, `single_robot_interface_passed=true`, and `failures=[]`.
- SONIC passed all 10 motions under the RMSE gate in `artifacts_simplified_full2_rerun/rmse_summary.csv`. HoloMotion passed 8/10; the remaining failures are the known out-of-distribution balance failures `dance_chicken_c03_neutral2s` and `dance_heart111_c01_neutral2s`.
- Hard gate 8 release proof passed in `artifacts_simplified_full2_rerun/release_validation.json`: direct release fell after `0.32s`, released SONIC `CONTROL` remained standing beyond 5 seconds with stable base height, and stopping control caused fall after `0.32s`.
- Hard gate 11 video proof passed with 10 MuJoCo-rendered comparison MP4s in `artifacts_simplified_full2_rerun/`.
...
- Existing partial logs under other `logs_*`/`artifacts_*` directories are development evidence only unless regenerated by `run.sh` with the current code and validated by `report.sh`.
