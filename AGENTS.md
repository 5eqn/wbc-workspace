...
- A bounded SONIC `dance_phony_c01_neutral2s` repeat probe stored under `/tmp/wbc-phony-c01-repeats` reproduced the intermittent failure on iteration 8/30: RMSE `0.226`, tracking delay `0.01s`, and minimum base height `0.085m`. Iterations 1-7 passed with RMSE about `0.120`-`0.128` and standing base height. Treat this as an intermittent timing/contact sensitivity with fresh raw evidence in `/tmp`, not retained benchmark evidence.
