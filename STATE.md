...
| `scripts/bfm_zero_latent_inspector.py` | BFM-ZERO-LATENT-INSPECTOR-ORIENTATION | Offline generator that validates both Stage 2 run orders and tiled videos, classifies orientation at the lowest-root raw sample in the first second, reuses the established BFM-Zero observation/checkpoint path, and computes shared deterministic PCA plus 256D spherical anomaly metrics. |
